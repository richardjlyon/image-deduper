...
use super::hash_processing::{compute_cryptographic_hash_with_timeout, compute_perceptual_hash_with_timeout};
...
pub fn process_image_batch(
    ...
    info!(
        ...
        end_mem / 1024 / 1024, mem_diff / 1024 / 1024
    );
...
pub fn process_images_in_batches(
    ...
    for (i, chunk) in images.chunks(batch_size).enumerate() {
        ...
        let (batch_results, errors) = process_image_batch(chunk, progress_counter, Some(config.clone()));
    ...
    info!("Final memory usage: before={}MB, after={}MB, diff=+{}MB", start_mem, end_mem, mem_diff);
