...
pub fn get_timeout_duration(file_ext: &str, operation: HashOperation) -> Duration {
    match operation {
        HashOperation::Cryptographic => {
            if ["raw", "raf", "dng", "cr2", "nef", "arw", "orf", "rw2", "nrw", 
                "crw", "pef", "srw", "x3f", "rwl", "3fr"].contains(&file_ext) {
    ...
    T: Send + 'static
{
    ...
    match rx.recv_timeout(timeout) {
        ...
        Err(e) => {
            ...
            info!("TIMEOUT: {} took too long for '{}'", operation_name, path_display);
            
            // Log the timeout error properly
            let timeout_err = std::io::Error::new(
                std::io::ErrorKind::TimedOut,
                format!("{} timed out after {} seconds: {:?}", operation_name, timeout_seconds, e)
            );
