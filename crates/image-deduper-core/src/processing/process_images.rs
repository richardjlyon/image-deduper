...
pub fn process_image_batch(
    ...
    // Log results to file
    info!("Batch completed: {} successful, {} errors in {:.2?}", 
          results.len(), error_counter.load(Ordering::Relaxed), batch_duration);
    
    // Log more detailed info to log file
    info!("Memory usage: before={}MB, after={}MB, diff=+{}MB",
...
pub fn process_images_in_batches(
    ...
    for (i, chunk) in images.chunks(batch_size).enumerate() {
        ...
        info!("Memory after batch {} processing: {}MB ({}MB change)", 
              i + 1, after_proc_mem, 
              if after_proc_mem > before_batch_mem { after_proc_mem - before_batch_mem } else { 0 });
        ...
        info!("Memory after adding to results: {}MB ({}MB change)", 
             after_extend_mem,
             if after_extend_mem > before_extend_mem { after_extend_mem - before_extend_mem } else { 0 });
        ...
        info!("Memory after cleanup: {}MB ({}MB change)", 
             after_cleanup_mem,
        ...
        if i % 2 == 0 { // Do this more frequently
    ...
    info!("Processing complete: {} successful, {} errors", results.len(), total_errors);
    info!("Total processing time: {:.2?}", batch_duration);
    info!("Final memory usage: before={}MB, after={}MB, diff=+{}MB", 
          start_mem, end_mem, mem_diff);
    info!("Memory per successful result: ~{}KB", 
             if results.len() > 0 { (mem_diff * 1024) / results.len() as u64 } else { 0 });
