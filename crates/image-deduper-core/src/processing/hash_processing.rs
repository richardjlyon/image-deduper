...
use super::timeout_utils::{execute_with_timeout, get_timeout_duration, HashOperation, extract_panic_info};
...
pub fn mark_as_problematic(path: &Path) {
    ...
    if let Ok(mut skip_list) = PROBLEMATIC_FILES.lock() {
        ...
        info!("Added {} to problematic files skip list (now {} entries)", 
              path_str, skip_list.len());
    }
}

/// Compute cryptographic hash with timeout protection
pub fn compute_cryptographic_hash_with_timeout(
    path: &Path
) -> Result<Blake3Hash> {
...
pub fn compute_perceptual_hash_with_timeout(
    path: &Path
) -> Result<PHash> {
