...
use image_deduper_core::{
    log_file_error, log_fs_modification, log_hash_error, send_direct_betterstack_log,
};
...
fn log_test_1() {
    ...
    log_hash_error!(
        ...
        Error::new(
            ErrorKind::TimedOut,
            "Perceptual hash timed out after 30 seconds: Timeout"
        )
    );
}
...
fn log_test_2() {
    ...
    log_file_error!(
        &file_path,
        "read",
        &Error::new(ErrorKind::TimedOut, "I/O error: Timeout")
    );
}
...
fn log_test_3() {
    ...
    log_fs_modification!(
        "create_directory",
        &fs_path,
        Some("Creating output directory for processed images")
    );
}
...
fn main() -> Result<()> {
    ...
    log_hash_error!(
        &test_path,
        Error::new(ErrorKind::TimedOut, "Function-based logging")
    );
    ...
    log_hash_error!(
        &test_path,
        Error::new(ErrorKind::TimedOut, "Macro-based logging")
    );
