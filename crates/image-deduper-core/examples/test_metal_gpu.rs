...
fn test_gpu_hash(image_path: &Path) {
    ...
    match image::open(image_path) {
        Ok(img) => {
            ...
            // Warm up
            let _ = perceptual::calculate_phash(&large_images[0]);
            
            let cpu_large_start = Instant::now();
            let cpu_large_hash = perceptual::calculate_phash(&large_images[0]);
            let cpu_large_duration = cpu_large_start.elapsed();
            println!("CPU hash (single large): {:016x} (took {:?})", cpu_large_hash.0, cpu_large_duration);
            ...
            if let Some(gpu_hash) = metal_phash::metal_phash(&img) {
                ...
                println!("\nTesting batch processing of {} large images:", num_large_images);
                ...
                // Calculate speedups
                let cpu_sequential_vs_parallel = cpu_batch_duration.as_nanos() as f64 / 
                                                cpu_parallel_duration.as_nanos() as f64;
                let cpu_vs_gpu = cpu_batch_duration.as_nanos() as f64 / 
                                 gpu_batch_duration.as_nanos() as f64;
                let cpu_parallel_vs_gpu = cpu_parallel_duration.as_nanos() as f64 / 
                                          gpu_batch_duration.as_nanos() as f64;
                ...
                println!("Small image:");
                println!("  GPU hash: {:016x} (took {:?})", gpu_hash.0, gpu_duration);
                
                if let Some(large_hash) = gpu_large_hash {
                    println!("Large image:");
                    println!("  GPU hash: {:016x} (took {:?})", large_hash.0, gpu_large_duration);
                    
                    let large_speedup = cpu_large_duration.as_nanos() as f64 / gpu_large_duration.as_nanos() as f64;
                    println!("  GPU speedup on large image: {:.2}x", large_speedup);
                }
                
                // Calculate difference for original small image
                let difference = hamming_distance(cpu_hash.0, gpu_hash.0);
