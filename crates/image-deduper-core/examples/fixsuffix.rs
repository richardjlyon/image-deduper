...
fn detect_image_format(path: &Path) -> Option<&'static str> {
    if let Ok(mut file) = fs::File::open(path) {
        ...
        if file.read_exact(&mut buffer).is_ok() {
            ...
            if buffer.starts_with(WEBP_MAGIC) && 
               buffer.len() >= 12 && 
               &buffer[8..12] == b"WEBP" {
                return Some("webp");
            }
...
fn rename_with_correct_extension(path: &Path, detected_format: &str, dry_run: bool) -> Result<PathBuf, String> {
    ...
    if new_path.exists() {
        return Err(format!("Cannot rename: target file already exists: {}", new_path.display()));
    }
...
fn process_image(path: &Path, dry_run: bool) -> Result<(), String> {
    ...
    if !path.exists() || !path.is_file() {
        return Err(format!("Path does not exist or is not a file: {}", path.display()));
    }
    ...
    if let Some(detected_format) = detect_image_format(path) {
        ...
        if !extension_matches_format(path, detected_format) {
            println!("Found file with incorrect extension: {} (actual format: {})", 
        ...
        println!("Warning: Could not determine format for file: {}", path.display());
    }
...
fn process_directory(dir_path: &Path, recursive: bool, dry_run: bool) -> Result<(), String> {
    ...
    for entry in walker.into_iter().filter_map(|e| e.ok()) {
        ...
        if let Some(ext) = path.extension() {
            let ext_str = ext.to_string_lossy().to_lowercase();
            if ["jpg", "jpeg", "png", "gif", "webp", "heic", "heif", "jpe", "jfif"].contains(&ext_str.as_ref()) {
                processed += 1;
                
                match process_image(path, dry_run) {
                    Ok(()) => {
                        if !extension_matches_format(path, &detect_image_format(path).unwrap_or("unknown")) {
                            renamed += 1;
                        }
                    },
    ...
    println!("Processed {} image files, renamed {} files with incorrect extensions", 
             processed, renamed);
...
fn print_usage() {
    ...
    println!("  fixsuffix --recursive ~/Pictures       # Process all images in Pictures recursively");
    println!("  fixsuffix --dry-run ~/Photos/*.jpg     # Preview changes without renaming");
}

fn main() {
    // Initialize logging
    if let Err(e) = logging::init_logger("logs") {
    ...
    println!("Mode: {}", if dry_run { "Dry run (no changes)" } else { "Live run" });
    println!("Processing {} path(s){}...", 
             paths.len(), 
