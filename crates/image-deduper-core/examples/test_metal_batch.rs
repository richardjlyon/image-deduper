use image_deduper_core::processing::{perceptual, metal_phash};
...
fn run_batch_test(images: &[PathBuf]) {
    ...
    println!("CPU processed {} images in {:?}", loaded_images.len(), cpu_duration);
    ...
    if let Some(_) = metal_phash::metal_phash(&loaded_images[0]) {
        ...
        println!("GPU processed {} images in {:?}", gpu_hashes.len(), gpu_duration);
        ...
        for i in 0..gpu_hashes.len().min(cpu_hashes.len()) {
            let distance = (gpu_hashes[i].0 ^ cpu_hashes[i].0).count_ones();
        ...
        let match_percent = (match_count as f64 * 100.0) / gpu_hashes.len().min(cpu_hashes.len()) as f64;
        ...
        for &(width, height) in &test_sizes {
            ...
            // CPU timing
            let cpu_start = Instant::now();
            let mut cpu_hash = perceptual::PHash(0);
            ...
            if let Some(hash) = gpu_hash {
                let distance = (cpu_hash.0 ^ hash.0).count_ones();
                let speedup = cpu_time.as_nanos() as f64 / gpu_time.as_nanos() as f64;
                
                println!("Image {}x{}: CPU={:?}, GPU={:?}, Speedup={:.2}x, Hamming distance={}", 
                        width, height, cpu_time, gpu_time, speedup, distance);
            } else {
                println!("Image {}x{}: CPU={:?}, GPU=Not available", width, height, cpu_time);
            }
            
            // For very large images, also test batches of 10 to see parallel performance
            if width >= 2048 && height >= 2048 {
                println!("  Testing batch processing for {}x{} images...", width, height);
